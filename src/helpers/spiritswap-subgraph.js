...
const getTokenPrice = async (pairAddress, priceIndex) => {
  try {
    ...
    `
    apiUrl = config.thegraph.spiritswap_url;
    let response = await thegraph.query(apiUrl, query);
    if(response && response._meta?.deployment != config.thegraph.spiritswap_deployment_id)
    {
      throw "Deployment ID of the SpiritSwap subgraph has been changed";
    }
    if(
